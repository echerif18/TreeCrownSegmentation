...
train_rgb_unet.py
─────────────────
Train Attention U-Net on RGB patches.

CLI:
    neon-train-unet --data-root data/splits/rgb_binary
"""
...
@click.option("--base-channels", default=64,    show_default=True, type=int)
...
def main(
    data_root, save_dir, img_size, base_channels, batch_size, num_workers,
    epochs, lr, weight_decay, patience, wandb_project, wandb_offline,
):
    """Train Attention U-Net on RGB binary patches."""
    _set_seed()
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    logger.info(f"Device: {device}")

    run_id  = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    run_dir = Path(save_dir) if save_dir else Path("runs") / f"{run_id}_attn_unet"
    ...
    wandb.init(project=wandb_project, name=f"{run_id}_attn_unet", config=cfg)
    ...
    for epoch in range(1, epochs + 1):
        ...
        if vl["val_iou"] > best_iou:
            best_iou = vl["val_iou"]
            torch.save({"model_state": model.state_dict(), "epoch": epoch, "iou": best_iou},
