...
def get_rgb_train_transforms(img_size: int = 320) -> A.Compose:
    return A.Compose([
        ...
        A.Affine(translate_percent=0.08, scale=(0.8, 1.2), rotate=(-20, 20), shear=(-8, 8), p=0.65),
        A.ElasticTransform(alpha=45, sigma=5, p=0.15),
...
def get_segformer_train_transforms(img_size: int = 512) -> A.Compose:
    ...
    return A.Compose([
        ...
        A.HorizontalFlip(p=0.5),
        A.VerticalFlip(p=0.5),
        A.Affine(translate_percent=0.08, scale=(0.85, 1.15), rotate=(-20, 20), shear=(-6, 6), p=0.65),
        A.ElasticTransform(alpha=45, sigma=5, p=0.12),
        A.RandomCrop(img_size, img_size, p=1.0),
        A.CoarseDropout(
        ...
        A.GridDistortion(num_steps=5, distort_limit=0.18, p=0.1),
    ])
